...
class Net_EarlyExit(nn.Module):
    def __init__(self, model=None,num_classes=200, exit_thresholds=[0.9, 0.92, 0.94, 0.96],planes=[256,512,1024,2048]):
        ...
        self.base_model_layers = nn.ModuleList([
    ...
    def forward(self, x):
        ...
        if self.inference_EE:
            ...
            for i, layer in enumerate(self.base_model_layers):
                x = layer(x)
...
class Blackbox(object):
    ...
    def from_modeldir(cls, model_dir, device=None, output_type='probs', **kwargs):
        ...
        print("Here let's change to Early exit")
